...
<i class="fas fa-file p-2"></i>
    ...
    <td>
        ...
        $deadlineDate = Carbon::parse($dokumen->deadline_date);
        @endphp
        <p class="text-xs font-weight-bold mb-0">{{ $deadlineDate->format('d/m/Y') }}</p>
        @if ($currentTime->lt($deadlineDate))
        @if ($deadlineDate->diffInDays() < 5) <p class="text-xs text-secondary mb-0 d-inline text-warning">{{
            $deadlineDate->diffInDays() }} Hari Lagi.</p>
            @else
            <p class="text-xs text-secondary mb-0 d-inline">{{ $deadlineDate->diffInDays() }} Hari Lagi.</p>
            @endif
            @elseif ($currentTime->eq($deadlineDate))
            <p class="text-xs text-secondary mb-0 d-inline text-yellow-500 text-warning">{{ $deadlineDate->diffInDays()
                }} Hari Ini.</p>
            @else
            <p class="text-xs text-secondary mb-0 d-inline text-danger">{{ $deadlineDate->diffInDays() }} Hari Lepas.
            </p>
            @endif
    </td>
    ...
    <td class="align-middle text-center">
        @php
        $submitted_at = Carbon::parse($dokumen->submitted_at);
        @endphp
